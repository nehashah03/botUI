...
✅ PRODUCTION-READY (SAFE UPGRADE)
- Keeps EXACT frontend contract (no breaking changes)
- Adds Cursor-style UX (progress %, substeps, smoother streaming)
- Adds optional image output (frontend can ignore safely)
- Adds timestamps for debugging

Pipeline:
initializing → steps → streaming → tools → sources → images → complete
"""
...
# =========================================================
# 🔹 PROCESSING STEPS (Cursor-style)
# =========================================================

PROCESSING_STEPS: List[dict] = [
    {"id": "step-1", "label": "Analyzing your query", "description": "Understanding intent"},
    {"id": "step-2", "label": "Searching documents", "description": "Retrieving context"},
    {"id": "step-3", "label": "Extracting insights", "description": "Processing data"},
    {"id": "step-4", "label": "Generating response", "description": "Composing answer"},
]


STEP_SUBSTEPS: Dict[str, List[str]] = {
    "step-1": ["Understanding intent", "Selecting tools", "Planning execution"],
    "step-2": ["Searching vector DB", "Ranking results", "Filtering context"],
    "step-3": ["Extracting entities", "Validating data", "Structuring insights"],
    "step-4": ["Drafting answer", "Formatting markdown", "Finalizing output"],
}
...
def _stage_for_step(step_id: str) -> str:
    return {
        ...
        "step-3": "extracting",
        "step-4": "generating",
    }.get(step_id, "processing")


def _map_steps(active_index: int, substeps: List[str]) -> List[dict]:
    result = []
    for i, step in enumerate(PROCESSING_STEPS):
        result.append({
            **step,
            "status": "complete" if i < active_index else "active" if i == active_index else "pending",
            "substeps": substeps if i == active_index else [],
        })
    return result
...
async def run_mock_stream(
    ...
    🔥 This simulates Cursor / ChatGPT streaming behavior:
    ...
    async def emit(payload: dict) -> None:
        """Central emitter (adds sessionId + timestamp)"""
        payload = {
            **payload,
    ...
    async def emit_activity(text: str):
        await emit({
            "event": "message",
            "type": "activity",
            "messageId": message_id,
    ...
    for i, step in enumerate(PROCESSING_STEPS):
        substeps = STEP_SUBSTEPS.get(step["id"], [])

        for j, sub in enumerate(substeps):
            percent = int(((i + (j + 1) / len(substeps)) / total_steps) * 100)

            await emit({
                "event": "step",
                "messageId": message_id,
                "step": {**step, "status": "active", "substeps": substeps[: j + 1]},
                "allSteps": _map_steps(i, substeps[: j + 1]),
            })
    ...
    # =====================================================
    # 🔹 OPTIONAL IMAGE OUTPUT (SAFE)
    # =====================================================
    await emit({
        "event": "message",
        "type": "images",  # frontend can ignore safely
        "messageId": message_id,
        "images": [
            {
